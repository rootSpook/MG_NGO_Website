...
export const contactSchema = z.object({
  ...
  email: z.string().trim().email("Geçerli bir e-posta adresi girin.").max(254, "E-posta adresi çok uzun."),
  phone: z.string().trim().max(20, "Telefon numarası en fazla 20 karakter olabilir.").optional().or(z.literal("")),
...
export const volunteerSchema = z.object({
  fullName: z.string().trim().min(2, "Ad Soyad en az 2 karakter olmalıdır.").max(100, "Ad Soyad en fazla 100 karakter olabilir."),
  email: z.string().trim().email("Geçerli bir e-posta adresi girin.").max(254, "E-posta adresi çok uzun."),
  phone: z.string().trim().max(20, "Telefon numarası en fazla 20 karakter olabilir.").optional().or(z.literal("")),
  city: z.string().trim().max(100, "Şehir en fazla 100 karakter olabilir.").optional().or(z.literal("")),
  motivation: z.string().trim().min(10, "Motivasyon en az 10 karakter olmalıdır.").max(2000, "Motivasyon en fazla 2000 karakter olabilir."),
});
...
export function contactFormToInput(values: ContactFormValues): ContactMessageInput {
  return {
    ...
    senderPhone: values.phone?.trim() || undefined,
